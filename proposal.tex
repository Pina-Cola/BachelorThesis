...
Doch die Zauberwürfel sind nicht nur interessante Puzzle -- sie bieten beispielsweise auch die Möglichkeit, sie maschinell mit Bild- und Farberkennung zu lösen. Außerdem kann man die Würfel als algebraische Strukturen darstellen.
...
noch nicht umfangreich als Gruppe untersucht wurde. Ziel dieser Arbeit ist es, das Wissen des \Tthree Würfels auf den \Ttwo Würfel zu übertragen. Der \Ttwo Würfel hat im Gegensatz zum \Tthree Würfel weniger Steine und keine Kantsteine -- dadurch ist der kleinere Würfel auch weniger komplex. Dafür kann man den \Ttwo Würfel aber aufgrund der fehlenden Mittelsteine rotieren und die Oberseite beliebig festlegen. In dieser Arbeit wird das Wissen der Gruppe des \Tthree Würfels übertragen und angepasst und somit der \Ttwo Würfel mit der Gruppentheorie dargestellt.
...
\item[\textbf{\ref{Kapitel_ValideKonfigurationen}}] Hier wird die Anzahl der möglichen Würfelkonfigurationen berechnet, also die Mächtigkeit der Trägermenge der Gruppe des \Ttwo Würfels.

\item[\textbf{\ref{Kapitel_Lösung}}] In diesem Kapitel werden verschiedene Lösungsansätze und Algorithmen erklärt. Die Lösung des Wüfels durch einen Menschen wird schrittweise erklärt und es gibt eine allgemeine Anleitung, mit der man den \Ttwo Würfel immer lösen kann.
...
Ein \Ttwo Würfel besteht aus acht Ecksteinen (links in Abbildung \ref{Abbildung_Eckstein}), die jeweils drei Farbflächen (rechts in Abbildung \ref{Abbildung_Eckstein}) haben. Ein \Ttwo Zauberwürfel hat also 24 Farbflächen. Die verschiedenen Farbpaare, die sich jeweils gegenüberliegen, sind weiß und gelb, rot und orange, grün und blau.
\begin{figure}[h]
\centering
...
\item[\Tthree Würfel] \ \\
Wesentlich bekannter als der \Ttwo \textit{Cube} ist der \Tthree \textit{Cube}. Er besteht aus 26 Steinen. In Abbildung \ref{Abbildung_3erWürfel} sieht man ihn links ungelöst und rechts in der Startkonfiguration.
...
\item Das Inverse Element für alle $z \in \mathbb{Z}_{\hspace*{-0.3em}\mod 6}$ ist definiert als $(6-z)$. Für 1 ist es also 5, für 2 ist es 4 und für 3 ist das neutrale Element die 3.
\end{itemize}

$(\mathbb{Z}_{\hspace*{-0.3em}\mod 6}, +)$ ist also eine endliche Gruppe.
...
In Abbildung \ref{Abbildung_Cayleygraph} sieht man den Cayleygraphen der Gruppe $(\mathbb{Z}_{\hspace*{-0.3em}\mod 6}, +)$ mit dem Erzeuger $1$.
...
\end{itemize}
Also operiert $(\mathbb{Z}_{\hspace*{-0.3em}\mod 6}, +)$ mit $-$ von rechts auf $\mathbb{Z}$.
...
Die Funktion $\pi$ sieht für diese Permutation (s. Tabelle) also so aus:
\begin{align*}
...
\pi = (1 \ 5 \ 2)\ (3 \ 4)
\end{align*}
Das wird so gelesen: Die 1 geht auf die 5, die 5 geht auf die 2, die 2 geht wieder auf die 1. Damit ist der erste Zykel geschlossen. Beim zweiten Zykel geht die 3 auf die 4 und die 4 auf die 3. $\pi$ besteht also aus zwei Zykeln: ein Zykel hat die Länge drei und der andere die Länge zwei. Die Zykel sind auch in Abbildung \ref{Abbildung_ZykelVonf} zu sehen.
\begin{figure}[H]
\centering
...
Eine symmetrische Gruppe $(S, \circ)$ ist eine Gruppe, die aus allen Permutationen (s. Kapitel \ref{Abschnitt_PermutationZykel}) der Menge $S$ besteht. 
\end{definition}
...
Die Einteilung der Würfelpositionen sieht man in den Abbildung \ref{Abbildung_SteinpositionNamen} und \ref{Abbildung_SteinpositionNamenFoldetOut}. Die einzelnen Steinpositionen werden mit den Kürzeln benannt, die \textit{up, down, right, left, front und back} beschreiben.
\begin{figure}[h]
\centering
...
Die Drehung der oberen Ebene sieht man grafisch dargestellt in Abbildung \ref{Abbildung_SteinpositionnamenNachU}.
\begin{figure}[h]
\centering
...
$(\Gtwo, \circ)$ ist keine kommutative Gruppe, da beispielsweise eine Rotation der rechten Ebene im Uhrzeigersinn ($R$) und eine Rotation der vorderen Ebene im Uhrzeigersinn ($F$) in umgekehrter Reihenfolge ein anderes Ergebnis haben. Das sieht man grafisch dargestellt in Abbildung \ref{Abbildung_WürfelNachFRundRF} oder wenn man es einfach händisch an einem Würfel ausprobiert.
\begin{figure}[h]
\centering
...
Um die Drehungen zu benennen, werden die Achsen des Würfels als $x, y$ und $z$ definiert. Das kann man in Abbildung \ref{Abbildung_Rotationsachsen} sehen.
\begin{figure}[h]
\centering
...
Anhand der Rotation $Z_r$, also einer Rotation des kompletten Würfels um die $z$-Achse um $90^\circ$ im Uhrzeigersinn, wird nun die Veränderung der Würfelpositionen gezeigt. Die Rotation $Z_r$ sieht man in Abbildung \ref{Abbildung_WürfelNachRotationUmZAchse}.
\begin{figure}[H]
\centering
...
In Abbildung \ref{Abbildung_GelöstnachFnachZL} sieht man dieses Beispiel nochmal grafisch: Links befindet sich der gelöste Würfel, in der Mitte der gelöste Würfel nach dem Zug $F$ und rechts der gelöste Würfel nach dem Zug $Z_rL$. Die beiden rechten Würfel sind in der gleichen Konfigurationen, aber anders gedreht.
\begin{figure}[h]
\centering
...
Grafisch dargestellt sieht man die Zykel des Zuges $LLFF$ in Abbildung \ref{Abbildung_ZykelVonLLFF}. 
\begin{figure}[H]
\centering
...
\end{figure}
In Zykelschreibeweise also $\sigma_{LLFF}=( \ \textit{dlf} \ \textit{ulb} \ \textit{urf})(\textit{ulf} \ \textit{dlb} \ \textit{drf} \ )$.
...
Außerdem kann man erkennen, dass sechs der acht Steine bei dem Zug $LLFF$ bewegt werden. Die anderen beiden (\textit{urb} und \textit{drb}) bleiben unverändert. 
...
Das liegt daran, dass man die Zykel in verschiedener Reihenfolge schreiben kann. So ist der Zug $U$ beispielsweise $ ( \ \textit{ulf} \ \textit{ulb} \ \textit{urb} \ \textit{urf} \ )$, aber auch $( \ \textit{urb} \ \textit{urf} \ \textit{ulf} \ \textit{ulb} \ )$.
...
Die Untergruppe $F$ wird erzeugt durch $\langle F \rangle = \{ F^n \mid n \in \mathbb{Z}\}$. Das bedeutet, dass $F = \{N, F^1, F^2, F^3, F^4, ...\}$. Es gibt auch unendliche zyklische Gruppen, diese hier sind aber endlich, da $N = FFFF$. Die Züge werden also (wie bereits in Kapitel \ref{Kapitel_WürfelAlsGruppe} beschrieben) modulo $4$ gerechnet.
...
Ein simples Beispiel sieht man in Abbildung \ref{Abbildung_CayleygraphF}: Der Cayleygraph der Untergruppe $F$ von $(\Gtwo, \circ)$, die durch $\{ F \}$  erzeugt wird.
...
In diesem Kapitel werden die validen Würfelkonfigurationen mithilfe der Gruppe $(\Gtwo,\circ)$ berechnet und mit der Mächtigkeit der Gruppe verglichen. Außerdem wird gezeig, dass für die Ausrichtung $x$ der Steine $\sum_{i= 1}^{8} \ x_i) \mod 3 = 0$ gilt.
...
Im Folgenden wird die Anzahl der möglichen Würfelkonfigurationen brechnet. Nicht jede der theoretisch möglichen Würfelkonfigurationen kann auch wirklich durch Ebenendrehungen erreicht werden. Wie viele Möglichkeiten es im \Ttwo Würfel gibt und welche davon man wirklich erreichen kann, wird im Folgenden berechnet. 
...
Da es keine Mittelsteine im Würfel gibt, reduziert die Rotation des Würfels die Möglichkeiten um den Faktor 24, denn es gibt $6$ Seiten, die jeweils $4$ Ausrichtungen haben können und so kommt man auch $6 \cdot 4 = 24$.
...
Allerdings berücksichtigt die Gruppe $(\Gtwo,\circ)$ nur die Permutationen der Steine und die Rotation des Würfels. Die Ausrichtung der Steine wir anhand eines Vekrotrs $x$ bestimmt. 

Das ganze kann man auch anhand der Gruppe $(\Gtwo,\circ)$ berechnen. Die Gruppe ist eine symmetrische Gruppe (s. Kapitel \ref{Abschnitt_SymmetrischeGruppe}), da sie aus allen Permutationen der Elemente der $8$-elementigen Menge $\{ \textit{ulb},  \textit{urb},  \textit{ulf},  \textit{urf},  \textit{dlb},  \textit{drb},  \textit{dlf},  \textit{drf} \}$ besteht. Dadurch ist die Mächtigkeit dieser Gruppe $|\Gtwo| = 8!=40\, 320$. \\
Es müssen aber auch die Äquivalenzrelationen der Rotationen von $(\Gtwo,\circ)$ berücksichtig werden. Es gibt Äquivalenzrelationen $\{{Z_r}, {Z_l}, {Y_r}, {Y_l}, {X_r}, {X_l}, N\}$, also für die Achsen $X, Y$ und $Z$, jeweils nach rechts und links. Da zwei Linksdrehungen und zwei Rechtsdrehungen äquivalent sind, gibt es also für jede Achse vier Rotationsmöglichkeiten. Das sind dann $3 \cdot 4 = 12$ Rotationen. Da aber zu jeder Achse zwei Seiten gehören, die getauscht werden können, sind es $2 \cdot 12 = 24$ Rotationen.
Obwohl die Gruppe die Mächtigkeit $40\, 320$ hat, muss dies aufgrund der Rotationsmöglichkeiten um den Faktor $24$ reduziert werden, wenn man die Anzahl der validen Würfelkonfigurationen anhand der Gruppe berechnen möchte. Dann erhält man $40\, 320 \cdot \frac{1}{24} = 1680$ mögliche Positionen der Ecksteine unter Berücksichtigung der Würfelrotationen. Diese Zahl ist recht niedrig, da die Ausrichtung der Steine noch nicht berücksichtigt wurde. \\
Die Ausrichtung der Steine wird durch den $8$-dimensionalen Vektor $x$ dargestellt. Jeder Eintrag in $x$ kann die Werte $0, 1$ oder $2$ annehmen. Das ergibt $3^8=6561$ Möglichkeiten der Steinausrichtung. Allerdings kann durch die Äquivalenzrelationen (und die Rotation des Würfels) ein Stein als richtig angenommen werden und somit kommt man auf $3^7=2187$ Möglichkeiten der Steinausrichtung. Somit gibt es $40\, 320 \cdot \frac{1}{24} \cdot 2187 = 3\, 674\, 160$ valide Würfelkonfigurationen. Da die Gruppe $(\Gtwo,\circ)$ allerdings nur die Position der Steine berücksichtig, ist die Mächtigkeit $|\Gtwo| 40\, 320$.
...
In Abbildung \ref{Abbildung_XnachZugR} in Kapitel \ref{Abschnitt_AusrichtungDerSteine} ist diese Situation für den Zug $R$ dargestellt. Das kann man auch anhand dieser Tabelle sehen: 
\vspace*{1em}
...
Wenn man nun die Kopfzeile der Tabelle und die unterste Zeile vergleicht, sieht man, dass \textit{urb}, \textit{urf}, \textit{dlb} und \textit{drb} wieder an ihrer Ausgangsposition sind. Die anderen vier Steine (\textit{ulf}, \textit{ulb}, \textit{urf} und \textit{ulf}) haben die Positionen gewechselt: \\
\\
...
In Abbildung \ref{Abbildung_AusrichtungEbene2} sieht man, wie der Würfel bei der Ausgangsposition (links) durch den Zug $\lbrack R, U \rbrack \lbrack R^{-1}, F \rbrack$ verändert wird. Die Steine der oberen Ebene werden dabei ausgerichtet.
...
Man kann $\lbrack R, U \rbrack \lbrack R^{-1}, F \rbrack$ auch als Zykel schreiben: $( \ \textit{ulb} \ \textit{ulf} \ \textit{urb} \ )$. 
%
%
%
%
...
Es muss also $x_{1-4} = (0,0,0,0)$ gelten und $\sigma_{ulb}=\textit{ulb}, \  \sigma_{urb} = \textit{urb}, \  \sigma_{ulf} = \textit{ulf}, \  \sigma_{urf}=\textit{urf}$ bzw. die Äquivalenzklassen $\delta_{Z_r}$ und $\delta_{Z_l}$ davon. 

Der Lösende sucht nun also Steine mit weißen Farbflächen und findet den weiß-orange-blauen Stein an der Position \textit{urf}. 
...
Dann führt man $D$ aus, bis der Stein sich unter seiner Zielposition befindet und dreht ihn mit $L, R, F$ oder $R$ neben den bereits vorhandenen, ersten Eckstein. Das kann man auch in Abbildung \ref{Abbildung_ZweiterEckstein} sehen.
...
Sollte die Farbfläche der Ecke unten am Würfel sein, so kann man sie falsch herum ausgerichtet mit der Technik aus Abbildung \ref{Abbildung_ZweiterEckstein} an die Position setzen. Dann befindet sich die weiße Farbfläche in der oberen Ebene, aber falsch herum ausgerichtet. Wenn das der Fall ist, kann man den Stein mit einem der Züge $L, R, F$ oder $B$ herausdrehen und mit $U$ in die untere Ebene schieben. Diesen Vorgang sieht man auch in Abbildung \ref{Abbildung_ZweiterEckstein2}. Von dort kann man ihn mit der oben beschriebenen Technik richtig ausgerichtet und positioniert einfügen (s. Abbildung \ref{Abbildung_ZweiterEckstein}). 
