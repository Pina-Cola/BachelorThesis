...
\\
\\
Daraus ergibt sich $Z_1 \sim Z_2 :\Leftrightarrow \ Z_1 = WZ_2$ mit $W$ als Element aus $\{{Z_r}, {Z_l}, {Y_r}, {Y_l}, {X_r}, {X_l}, \epsilon\}$, wobei $\epsilon$ die \glqq leere\grqq \ Rotation darstellt, also keine Rotation des Würfels. \\
\\ 
...
\end{align*}
Das gilt, wenn $W_2$ das Inverse von $W_1$ ist. Das Inverse ist die Rotation um die gleiche Achse, aber in die andere Richtung. Von $Z_r$ beispielsweise ist das Inverse $Z_l$. \\
Ich schreibe das Inverse von $W$ als $W^{-1}$. Dann gilt: 
\begin{align*}
