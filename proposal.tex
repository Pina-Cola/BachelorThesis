...
\sigma_R & =\ ( \ \textit{urb} \ \textit{drb} \ \textit{drf} \ \textit{urf} \ ) 
\end{align*}
Jeder Grundzug ist demnach als ein $4$-Zykel definiert. Jeder $4$-Zykel kann als Produkt aus drei $2$-Zykeln geschrieben werden. 
...
Bei dem \Tthree Würfel ist die Anzahl der $2$-Zykel jeder Permutation gerade, deshalb gibt es keinen Zug, der genau zwei Steine im Würfel tauscht \cite{TD}. Im Gegensatz dazu kann die Anzahl der $2$-Zykel der Zykel des \Ttwo Würfels ungerade sein und es gibt für diesen Würfel Algorithmen, die die Position von genau zwei Steinen tauschen. Diese Algorithmen sind in Kapitel \ref{Abschnitt_AlgorithmenZweiteEbene} beschrieben. 
