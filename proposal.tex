...
Es gibt Links- und Rechtsoperationen. Da im Folgenden nur Rechtsoperationen genutzt werden, werden auch nur diese hier definiert.
...
Wenn der Zauberwürfel in einer Konfiguration $C=(\sigma, x)$ aus der Menge der Konfigurationen $M_C$ ist, wird er durch das Ausführen eines Zuges $Z \in \Gtwo$ in eine neue Konfiguration gebracht. Diese Konfiguration wird als $(C \cdot Z) \in M_C$ geschrieben. 


Definition einer Gruppenoperation mit der Gruppe $(\Gtwo, \circ)$ und der Konfiguration $C$:
\begin{itemize}
\item $\cdot: M_C \times \Gtwo \rightarrow M_C$ mit $(c, g) \rightarrow c \cdot g $
\item $c \cdot N = x$ für alle $c \in C$ und das neutrale Element $N \in \Gtwo$  
\item $c \cdot (u \circ v) = (c \cdot h) \cdot v$ für alle $u, v \in \Gtwo$ und $c \in M_C$
\end{itemize}
