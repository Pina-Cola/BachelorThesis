...
Zwei Züge $Z_1$ und $Z_2$ gelten als gleich, wenn sie mit gleicher Ausgangskonfiguration des Würfels die gleiche Zielkonfiguration hervorrufen.
Ein Zug besteht aus einem oder mehreren Grundzügen ($U, D, F, B, L, R$).
Das mehrfache Ausführen der Züge ($U, D, F, B, L, R$) kann mit der Exponentenschreibweise dargestellt werden. So wird beispielsweise $RR$ (zwei Drehungen der rechten Ebene im Uhrzeigersinn) auch als $R^2$ geschrieben.

Wenn eine Ebene vier mal hintereinander gedreht wird, ist der Würfel wieder in der vorherigen Position. Wenn der Würfel in einer Konfiguration $C = (\sigma, x)$ ist und ein Zug $Z \in \{ U^4, D^4, R^4, L^4, F^4, B^4\} $ ausgfeührt wird, ist die Folgekonfiguration wieder $C$.
\begin{align*}
...
Da die Anzahl der möglichen Züge im Gegensatz zur Anzahl der validen Würfelkonfigurationen (s. Kapitel \ref{Kapitel_ValideKonfigurationen}) unendlich ist, gibt es viele Fälle, in denen zwei Züge die gleiche Folgekonfiguration hervorrufen und somit als gleich gelten.
...
Somit gilt dann auch hier $RRRR=R^4=N$ (für $R \in \{{Z_r}, {Z_l}, {Y_r}, {Y_l}, {X_r}, {X_l} \}$) und $R^0=N$ (für alle Rotationen). ($N$ ist das neutrale Element bzw. der leere Zug und $R$ ist eine beliebige Rotation des Würfels.) 
...
In dem Fall des \Ttwo Würfels handelt es sich um eine Relation von zwei Zügen $Z_1, Z_2$. 
...
Daraus ergibt sich $Z_1 \sim Z_2 :\Leftrightarrow \ Z_1 = WZ_2$ mit $W$ als Element (oder Kombination von Elementen) aus $\{{Z_r}, {Z_l}, {Y_r}, {Y_l}, {X_r}, {X_l}, N\}$ -- wobei $N$ die \textit{leere} Rotation darstellt, das entspricht keiner Rotation des Würfels. 
...
\begin{align*}
Z_1 \sim_R Z_2 : & \Leftrightarrow  Z_1 = WZ_2
\end{align*}
...
\end{align*}
Es gilt somit die Reflexivität für $\sim$, da für $Z_1 \sim Z_2$ mit $W=N$ immer $Z_1 = Z_2$ gilt.
...
%
%
\subsection*{Gruppe} \addcontentsline{toc}{subsection}{\protect\numberline{}Gruppe}
...
Wenn eine Ebene vier mal gedreht wird, ist der Würfel wieder in der vorherigen Position. Somit gilt beispielsweise $RRRR=R^4=N$, wobei $N$ für das neutrale Element (bzw. den leeren Zug) steht. Das gilt für alle Züge des Würfels:
...
Es gilt demnach für alle Züge $Z \in \Gtwo$: 
\begin{align*}
Z^0 & =N \\
Z & =Z^1 \\
ZZ & =Z^2 \\
ZZZ & =Z^3 \\
\end{align*}
Für \textit{einelementige} Züge $Z$ ($U, D, F, B, L$ oder $R$) gilt auch $ZZZZ =Z^4=N=Z^0$.
