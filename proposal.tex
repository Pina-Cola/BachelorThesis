...
Ich orientiere mich dabei vorallem an den Papern \glqq Group Theory an the Rubik's Cube\grqq \ von Janet Chen \cite{JC} und \glqq Group Theory via Rubik's Cube\grqq \ von Tom Davis \cite{TD}.
...
Im Folgenden wird die Definition der Gruppe des \Tthree Würfels aus dem Paper \glqq Group Theory and the Rubik's Cube\grqq \  von Janet Chen \cite{JC} als Gruppe des 2x2x2-Würfels umgesetzt. Die Gruppe von Janet Chen wird hier als $(\Gthree, \circ)$ bezeichnet, auch wenn er sie als $(G, *)$ bezeichnet hat. Der Namen der Gruppe wird geändert, um klar zwischen dem \Ttwo  und dem \Tthree Würfel zu differenzieren. Das Symbol des Operators wird verändert, um Verwechslungen mit dem Multiplikationsoperator zu vermeiden. \\
\\ 
...
Wenn $n=0$ ist, hat man $Z^0=N$, was den leeren Zug (bzw. das neutrale Element) repräsentiert. Der Würfel wird dabei nicht verändert und somit wird die Würfelposition bei jedem Zug $Z \in \Gtwo$ mit $Z^0$ \glqq wiederholt\grqq . \\
\\
