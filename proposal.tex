...
Ein solcher Baum enthält eine Konfiguration $C_0$ als Wurzel. Von diesem Startknoten ausgehend gibt es eine Kante für jeden möglichen Zug des Würfels. Diese Kanten führen jeweils zu einer Folgekonfiguration $C_1$. So folgt dann auf jeden Knoten $C_i$ ein Knoten $C_{i+1}$ in der nächsten Ebene. Zu einem Knoten mit der Konfiguration $C_i=(0,1)$ führt der Pfad im Baum, der einen Lösungsweg repräsentiert. Die Konfiguration $C_i$ befindet sich in der $i$-ten Ebene des Baums und wurde nach $i$ Zügen erreicht. So läaast sich die Länge des optimalen Lösungswegs auslesen.
...
Allerdings muss bei der Suche 
%Der Baum der \textit{Half-Turn} Metrik ist kleiner als der Baum der \textit{Quarter-Turn} Metrik. Wird der Baum der \textit{Half-Turn} Metrik modifiziert, kann so auch die Länge der optimalen Lösung der \textit{Quarter-Turn} Metrik abgelesen werden. 
