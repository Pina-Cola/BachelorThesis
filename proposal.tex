...
Bei einem beliebigen Zug $Z \in G_{2x2x2}$ gelangen wir also irgendwann in eine Würfelposition, die sich wiederholt. Die wiederholende Würfelposition tritt zuerst nach $n \in \mathbb{N}$ Wiederholungen auf und das zweite Mal nach $m \in \mathbb{N}$ Wiederholungen. Das sieht dann so aus: $Z^n=Z^m$ mit $n < m$. \\
Also gilt $Z^{n-1} \neq Z^{m-1}$, da $m$ das zweite Vorkommen der Position repräsentiert. \\
Wenn $n=0$ ist, haben wir $Z^0=N$, was den leeren Zug (bzw. das neutrale Element) repräsentiert. Der Würfel wird dabei nicht verändert und somit wird die Würfelposition bei jedem Zug $Z \in G_{2x2x2}$ mit $Z^0$ \glqq wiederholt \grqq .
