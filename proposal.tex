...
Im Folgenden wird gezeigt, dass $(G_{2x2x2}, \circ)$ eine Gruppe ist, indem $(G_{2x2x2}, \circ)$ bezüglich der Gruppenkriterien untersucht wird: \\
\begin{description}
...
\\
Das neutrale Element $N$ muss aus der Menge $G_{2x2x2}$ der Züge sein und es muss gelten: $N \circ Z_1 = Z_1 \circ N = Z_1$. Somit ist das neutrale Element der Gruppe $(G_{2x2x2}, \circ)$ der \textit{leere} Zug. Es werden also keine der Ebenen des Würfels gedreht. Wenn man also einen Zug $Z$ ausführt und dann den Zug $N$, bedeutet das so viel wie \glqq erst $Z$ ausführen und dann nichts \grqq , was das gleiche ist wie $Z$ auszuführen.


\item [Existenz eines inversen Elements $Z_1^{-1}$] \ \\
$\forall \  Z_1 \in G_{2x2x2},\ \exists \  Z_1^{-1} \in G_{2x2x2}.  \ \ Z_1 \circ Z_1^{-1} = Z_1^{-1} \circ Z_1 = N$  \\
\\
...
\\
$(G_{2x2x2}, \circ)$ ist keine kommutative Gruppe, da beispielsweise eine Rotation der rechten Ebene im Uhrzeigersinn ($R$) und eine Rotation der vorderen Ebene im Uhrzeigersinn ($F$) in umgekehrter Reihenfolge ein anderes Ergebnis haben. Das sieht grafisch dargestellt so aus: \\
\begin{figure}[H]
\centering
...
\\
Definition einer Gruppenoperation mit der Gruppe $(G_{2x2x2}, \circ)$ und dem Menge $C$:
\begin{itemize}
...
\\
Die Exponentenschreibweise kann man für alle Züge aus $G_{2x2x2}$ anwenden, auch wenn sie mehr als eine Ebene rotieren. Beispielsweise mit $LLFF$. $(LLFF)^2$ ist dann $LLFFLLFF$ und $(LLFF)^3$ ist $LLFFLLFFLLFF$, was wieder die Ausgangsposition ergibt. \\
\\
...
${\forall \ Z \in G_{2x2x2}\ mit \ \sigma_Z=(i_1 \ i_2 \ ... \ i_n), n \in \mathbb{N} \ . \  Z^n=1 }$. \\
\\
...
\\
Jede Gruppe G mit neutralem Element $N$ hat die beiden trivialen Untergruppen ${H_N = \{N\}}$ und $H_G=G$ (hier: $G=G_{2x2x2}$). Also sind $H_N$ und $H_G$ die trivialen Untergruppen von $G_{2x2x2}$. ($H_N \leqslant G_{2x2x2}$ und $H_G \leqslant G_{2x2x2}$)\\
