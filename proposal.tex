...
In diesem Beispiel (Abbildung \ref{Abbildung_LösungMensch0}) wird der Würfel mit dem Zug $FUBRF^{-1}$ verdreht und so gelöst, wie ein Mensch den \textit{Cube} lösen würde. Der Würfel befindet sich dann in der Konfiguration $C = ((\textit{ulf} \ \textit{dlf} \ \textit{urf} \ \textit{dlb}),(2,2,0,0,2,2,1,0))$.
In den Abbildungen wurde der Würfel noch um $180^\circ$ um die $z$-Achse rotiert. Dann ist die Würfelkonfiguration $C=((\textit{ulb} \ \textit{urb} \ \textit{urf} \ \textit{drb} \ \textit{drf} \ \textit{dlf} \ \textit{ulf} \ \textit{dlb} ),(0,2,0,2,1,2,0,2))$.
...
Für die Lösung der oberen Ebene muss $x_{1-4} = (0,0,0,0)$ gelten und $\sigma_{ulb}=\textit{ulb}, \  \sigma_{urb} = \textit{urb}, \  \sigma_{ulf} = \textit{ulf}, \  \sigma_{urf}=\textit{urf}$. 
